...
//-----------------------------------------------------------------------------
VOID DXUtil_LaunchReadme( HWND g_hWnd, LPCTSTR strLoc )
{
#ifdef UNDER_CE
    // This is not available on PocketPC
    MessageBox( g_hWnd, TEXT("For operating instructions, please open the ")
    ...
    for( int i=0; i < NUM_FILENAMES; i++ )
    {
        ...
        if( bFound )
        {
            ...
            if (hInstShell32 != NULL)
            {
                ...
                {
                    if( pShellExecute( g_hWnd, TEXT("open"), strReadmePath, NULL, NULL, SW_SHOW ) > (HINSTANCE) 32 )
                        bSuccess = true;
                }
    ...
    MessageBox( g_hWnd, TEXT("Could not find readme"), 
                TEXT("DirectX SDK Sample"), MB_ICONWARNING | MB_OK );
}
